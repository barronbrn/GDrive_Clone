<aside class="w-64 bg-white flex flex-col p-6 border-r border-gray-200">
    ...
    <div x-data="{ open: false }" class="relative">
        @auth
            ...
            <div x-show="open" @click.outside="open = false" x-cloak class="absolute z-10 mt-2 w-full bg-white rounded-md shadow-lg border">
                <a href="#" @click.prevent="showUploadFileModal = true; open = false" class="block px-4 py-3 text-sm text-gray-700 hover:bg-gray-100"><div class="flex items-center space-x-2"><span>Upload File</span></div></a>
                <a href="#" @click.prevent="showCreateFolderModal = true; open = false" class="block px-4 py-3 text-sm text-gray-700 hover:bg-gray-100"><div class="flex items-center space-x-2"><span>New Folder</span></div></a>
            </div>
        @else
            <a href="{{ route('login') }}" class="w-full flex items-center justify-center space-x-2 bg-bri-blue text-white font-semibold py-3 px-4 rounded-lg hover:bg-bri-blue-dark transition-colors"><span>Login to Create</span></a>
        @endauth
    </div>
    ...
    <nav class="mt-10 flex-1">
        <ul class="space-y-2">
            ...
            @auth
                <li><a href="{{ route('profile.edit') }}" class="flex items-center space-x-3 p-3 rounded-lg transition-colors {{ request()->routeIs('profile.edit') ? 'text-bri-blue font-semibold bg-blue-100' : 'text-gray-500 hover:text-gray-800 hover:bg-gray-100' }}"><span>Profile</span></a></li>
            @endauth
        </ul>
    </nav>
</aside>